...
app.post('/api/admin/import-stores', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    if (req.userType !== 'admin') {
      return res.status(403).json({ message: 'Acesso negado' });
    }
    ...
    const data = xlsx.utils.sheet_to_json(worksheet, { header: ['username', 'password', 'storeName'], range: 1 });
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      const rowNumber = i + 2; // +2 porque começamos na linha 2 (linha 1 é header)

      try {
        // Validar dados
        if (!row.username || !row.password || !row.storeName) {
          results.errors.push({
            row: rowNumber,
            username: row.username || 'N/A',
            error: 'Dados incompletos (username, password ou storeName em falta)'
          });
          continue;
        }

        // Verificar se username já existe
        const existingUser = await pool.query('SELECT * FROM users WHERE email = $1', [row.username]);
        if (existingUser.rows.length > 0) {
          results.errors.push({
            row: rowNumber,
        ...
        // Hash da password
        const passwordHash = await bcrypt.hash(row.password.toString(), 10);

        // Criar utilizador
        const userResult = await pool.query(
          'INSERT INTO users (email, password_hash, user_type) VALUES ($1, $2, $3) RETURNING id',
          [row.username, passwordHash, 'store']
        );
        ...
        // Criar loja
        await pool.query(
          'INSERT INTO stores (user_id, store_name) VALUES ($1, $2)',
          [userId, row.storeName]
        );
    ...
    res.json({
      message: `Importação concluída. ${results.success.length} loja(s) criada(s), ${results.errors.length} erro(s).`,
    ...
    res.status(500).json({ message: 'Erro ao processar ficheiro Excel' });
  }
});
